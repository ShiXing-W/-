...
class _Gambling():
    ...
    def Earning1(self,agvNo):    #计算收益
        earns={}    #收益字典
        for key in self.Pool.keys():   #遍历任务池中的任务
            if self.Pool[key].expectTime-self.t<0:
                earn = 3 * (abs(self.F[agvNo].position[0] - self.Pool[key].start[0]) + abs(
                    self.F[agvNo].position[1] - self.Pool[key].start[1])) // A_v
                +7 * (abs(self.Pool[key].expectTime - self.t - (
                            abs(self.F[agvNo].position[0] - self.Pool[key].start[0]) + abs(
                        self.F[agvNo].position[1] - self.Pool[key].start[1])) // A_v))
            else:
                earn=5*(abs(self.F[agvNo].position[0]-self.Pool[key].start[0])+abs(self.F[agvNo].position[1]-self.Pool[key].start[1]))//A_v
                +5*(abs(self.Pool[key].expectTime-self.t-(abs(self.F[agvNo].position[0]-self.Pool[key].start[0])+abs(self.F[agvNo].position[1]-self.Pool[key].start[1]))//A_v))
            earns[key]=earn
        return earns
    ...
    def Earning(self,agvNo):    #计算收益
        earns={}    #收益字典
        for key in self.Pool.keys():   #遍历任务池中的任务
            earn=abs(self.F[agvNo].position[0]-self.Pool[key].start[0])+abs(self.F[agvNo].position[1]-self.Pool[key].start[1])//A_v
            earns[key]=earn
        return earns
    ...
    def Game(self):
        ...
        for n in range(len(row_ind)):
            ...
            if len(self.F[no].currentTasks)==1:
                self.T[tno].startTime=self.t+(abs(self.F[no].position[0] - self.T[tno].start[0])+ abs(self.F[no].position[1] - self.T[tno].start[1]))//A_v
                ...
                self.T[tno].startTime=self.F[no].endTime[-1]+(abs(self.F[no].position[0] - self.T[tno].start[0])+ abs(self.F[no].position[1] - self.T[tno].start[1]))//A_v
                self.T[tno].endTime = self.T[tno].startTime+self.T[tno].distance//A_v
